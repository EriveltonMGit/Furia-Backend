...
exports.verifyIdentity = [
    ...
    async (req, res) => {
        try {
            ...
            // Agora req.user estará disponível devido ao middleware 'protect'
            if (req.user && req.user.id) {
                // Atualize o documento do usuário no Firestore indicando que a face foi verificada
                const userRef = db.collection('users').doc(req.user.id);
                await userRef.update({ faceVerified: faceVerified });
                return res.json({ success: true, faceVerified: faceVerified });
            } else {
                // Esta condição não deve ser atingida se o middleware 'protect' estiver configurado corretamente
                console.warn("Usuário não autenticado ao verificar a identidade (erro inesperado).");
